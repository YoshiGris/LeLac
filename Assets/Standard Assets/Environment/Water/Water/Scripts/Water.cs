...
namespace UnityStandardAssets.Water
{
    ...
    public class Water : MonoBehaviour
    {
        ...
        public void OnWillRenderObject()
        {
			...
			{
				//refractionCamera.useOcclusionCulling = false;reflectionCamera.useOcclusionCulling = false; //OCCLUSION DESACTIVE HUHU :3
			}
            UpdateCameraModes(cam, reflectionCamera);
            UpdateCameraModes(cam, refractionCamera);

            // Render reflection if needed
            if (mode >= WaterMode.Reflective && cubemap)
            {
            ...
            // Render refraction
            if (mode >= WaterMode.Refractive && cubemap)
            {
